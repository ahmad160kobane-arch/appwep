...
export default function ContentCard({ item }: Props) {
  ...
  return (
    <>
      <Link href={isPremium ? href : '#'} onClick={handleClick} className="block w-full group cursor-pointer card-hover">
        <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-light-input dark:bg-dark-input">
          ...
          {/* Premium lock badge */}
          {!isPremium && (
            <div className="absolute top-2 left-2 flex items-center gap-0.5 bg-amber-500/80 backdrop-blur-sm px-1.5 py-0.5 rounded-md">
              <svg className="w-2.5 h-2.5 text-white fill-white" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
              </svg>
              <span className="text-[8px] text-white font-bold">بريميوم</span>
            </div>
          )}
          ...
          {/* Rating badge */}
          {item.rating && (
            <div className="absolute top-2 left-2 flex items-center gap-0.5 bg-black/60 px-1.5 py-0.5 rounded-md" style={!isPremium ? { left: 'auto', right: '2.5rem' } : {}}>
      ...
      {showAuthPrompt && (
        <AuthPrompt type="premium" onClose={() => setShowAuthPrompt(false)} />
      )}
    </>
  );
}
