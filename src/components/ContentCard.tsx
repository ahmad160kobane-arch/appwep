      ...
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-light-input dark:bg-dark-input">
        {item.poster && !imgError ? (
          <img
            src={item.poster}
            alt={item.title}
            className={`w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 ${!isPremium ? "brightness-75" : ""}`}
        ...
        {/* Type badge */}
        <div
          className={`absolute top-2 right-2 px-1.5 py-0.5 rounded-md text-[9px] font-bold ${
        ...
        {!isPremium && !loading && (
          <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-black/70 backdrop-blur-sm px-1.5 py-1 rounded-md">
            <svg
            ...
            <span className="text-[8px] text-amber-400 font-bold">بريميوم</span>
          </div>
        )}

        {/* Play button on hover — only for premium */}
        {isPremium && (
        ...
        {/* Lock overlay on hover for non-premium */}
        {!isPremium && !loading && (
